...
object DataExhaustTest extends Serializable {

  def main(args: Array[String]): Unit ={
    val config = testModelConfig()
    implicit val (spark, sc, fc) = DashboardUtil.Test.getSessionAndContext("DataExhaustTest", config)
  ...
  def test(config: Map[String, AnyRef])(implicit spark: SparkSession, sc: SparkContext, fc: FrameworkContext): Unit = {
    DataExhaustModel.parseConfigAndProcessData(System.currentTimeMillis(), config)
  }
