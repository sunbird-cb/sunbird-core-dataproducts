...
object RozgarReportTestNew extends Serializable {

  def main(args: Array[String]): Unit = {

    val config = testModelConfig()
    implicit val (spark, sc, fc) = DashboardUtil.Test.getSessionAndContext("RozgarReportTestNew", config)
  ...
  def test(config: Map[String, AnyRef])(implicit spark: SparkSession, sc: SparkContext, fc: FrameworkContext): Unit = {
    RozgarReportModelNew.processUserEnrolmentData(System.currentTimeMillis(), config)
  }
