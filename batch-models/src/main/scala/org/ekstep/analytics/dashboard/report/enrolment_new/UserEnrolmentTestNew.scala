...
object UserEnrolmentTestNew extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    implicit val (spark, sc, fc) = DashboardUtil.Test.getSessionAndContext("UserEnrolmentTestNew", config)
  ...
  def test(config: Map[String, AnyRef])(implicit spark: SparkSession, sc: SparkContext, fc: FrameworkContext): Unit = {
    UserEnrolmentModelNew.processUserEnrolmentData(System.currentTimeMillis(), config)
  }
