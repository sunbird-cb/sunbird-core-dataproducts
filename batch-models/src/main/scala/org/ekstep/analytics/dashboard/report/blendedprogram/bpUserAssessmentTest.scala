...
object bpUserAssessmentTest extends Serializable {

  def main(args: Array[String]): Unit = {
    val config = testModelConfig()
    implicit val (spark, sc, fc) = DashboardUtil.Test.getSessionAndContext("bpUserAssessmentTest", config)
  ...
  def test(config: Map[String, AnyRef])(implicit spark: SparkSession, sc: SparkContext, fc: FrameworkContext): Unit = {
    bpUserAssessmentModel.processBPUserAssessmentData(System.currentTimeMillis(), config)
  }
