...
class Summary2(val firstEvent: WFSInputEvent) {
...
//    val object_rollup: Option[RollUp] = if(firstEvent.`object`.nonEmpty) firstEvent.`object`.get.rollup else None
    ...
    var CHILDREN: Buffer[Summary2] = Buffer()
    var PARENT: Summary2 = null
    ...
    def addChild(child: Summary2) {
        this.CHILDREN.append(child);
    }

    def addParent(parent: Summary2, idleTime: Int) {
        this.PARENT = parent;
        // Add first event of child to parent
        this.PARENT.add(this.firstEvent, idleTime)
    }

    def checkStart(`type`: String, mode: Option[String], summEvents: Buffer[MeasuredEvent], config: Map[String, AnyRef]): Summary2 = {
...
//            if(this.PARENT != null) return PARENT else return this;
    ...
    def checkEnd(event: WFSInputEvent, idleTime: Int, config: Map[String, AnyRef]): Summary2 = {
...
//        }
    ...
    def getSimilarEndSummary(event: WFSInputEvent): Summary2 = {
        val mode = if(event.edata.mode == null) "" else event.edata.mode
        if(StringUtils.equalsIgnoreCase(this.`type`, event.edata.`type`) && StringUtils.equals(this.mode.get, mode)) {
            return this;
        }
    ...
    def checkSimilarity(summ: Summary2): Boolean = {
